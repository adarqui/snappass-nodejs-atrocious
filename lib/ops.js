...
var set_password = function(state, password, ttl, cb) {
    var key = uuid.v4();
    state.redis.set(wrap_key(key), password, function redisSetKeyAndPassword(err, data) {
...
var get_password = function(state, key, cb) {
    state.redis.get(wrap_key(key), function redisGetPasswordFromKey(err, password) {
        if (err == null) {
